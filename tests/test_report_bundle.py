...
def test_real_task_report_bundle_is_self_contained_for_reporting(tmp_path: Path) -> None:
    ...
    assert (bundle_dir / "README_汇报说明.txt").exists()
    assert (bundle_dir / "docs" / "工程方案摘要.md").exists()
    assert (bundle_dir / "docs" / "运行命令.md").exists()
    assert (bundle_dir / "docs" / "汇报检查清单.md").exists()
    ...
    assert "overlap 不是固定先验" in (bundle_dir / "README_汇报说明.txt").read_text(encoding="utf-8")
